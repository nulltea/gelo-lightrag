...
| **Static-weight obfuscation** | `m2_7/run_static_attacks.py`, `m2_7/run_ima_embedrow_attacks.py` | Offline attacker has both plain and obfuscated GGUF weights (θ, θ̃). Recovers the τ-permutation that maps plain → obfuscated token ids. | **No** — GELO does not obfuscate the embedding table or any weights. Weights are clear; the protocol's secret is the per-forward mask `A`. Listed here for the path-2 M2.7 GGUF-obfuscation research; not in the GELO acceptance gate. |
