...
## Erratum — 2026-05-26 late audit

The PDF (`2603.01499v2.pdf`, Algorithm 2 line 6, page 9) includes `R̂_qk` on the K side: `W̃_k = Q̂_k W_k R̂_qk Ĥ_qk^-1 Ẑ_block^T`. This handoff's earlier A1 description (`Ĥ^-1 Ẑ^T`, no `R̂`) was a misread. The built `PAPERLIT` cell and the row-split numbers below therefore characterize a **no-R beyond-paper hardening variant**, not true paper-literal Alg2.

Code has since been corrected so `--alg2-paper-literal-k` means true-paper `R H^-1 Z^T`; the old no-R behavior is available only through `--alg2-paper-literal-k-no-r`. Treat this handoff as historical context plus no-R evidence, not as a paper-faithful result.
...
1. **Our deployed Alg2 was understating AloePri's actual defense by 7–40 pp** across surfaces × layers. Two construction deviations vs paper Algorithm 2:
   - **A1** — `k_matrix = R̂_qk · Ĥ⁻¹ · Ẑ` (our deployment) vs paper's true `R̂ · Ĥ⁻¹ · Ẑᵀ`; this handoff misread it as `Ĥ⁻¹ · Ẑᵀ` with no R̂.
   - **A2** — Û_vo QR-stabilised + 0.05 σ perturbation (our deployment) vs paper's raw `N(0, 1/d_head)` Gaussian.
2. **No-R experimental A1+A2 cell built and remeasured** (`untied-keymat-h128-pi-noise-ae1.0-ah0.2-alg2-matrix-gamma-hadamard-uvo-PAPERLIT-bf16-native.gguf`). Ridge defense delta on the §5.4-bounded `kqv_out` output surface at L=5 grows from 0.5 pp (default Alg2) to 40 pp (no-R experimental). At L=0 it grows from 14 pp to 50 pp. Score-surface `kq` defense delta at L≥5 grows from ~1 pp to 16–31 pp.
...
- `build_layer_keys()` — added `paper_literal: bool = False` flag. When True, incorrectly set `k_matrix = Ĥ⁻¹ · Ẑᵀ` instead of the PDF's true `R̂_qk · Ĥ⁻¹ · Ẑᵀ`; corrected code now uses the true formula for `--alg2-paper-literal-k`, AND passes the flag through to `generate_u_vo`. Comment block documents the `‖Ẑ² − I‖` score-surface residual that the no-R experimental construction accepts as part of `e_C^attn`.

### `python/aloepri-llm/obfuscate_qwen3_gguf.py`

- Historical note: the original `--alg2-paper-literal` implementation in this handoff was later corrected; no-R behavior now requires `--alg2-paper-literal-k-no-r`.
...
| Artefact | Path | Size |
|---|---|---|
| No-R experimental Q3-4B obf cell | `/home/timo/.cache/huggingface/path-2-aloepri/qwen3-4b/untied-keymat-h128-pi-noise-ae1.0-ah0.2-alg2-matrix-gamma-hadamard-uvo-PAPERLIT-bf16-native.gguf` | 9.7 GB |
| No-R experimental cell key | `…-PAPERLIT-bf16-native.gguf.key.npz` | 536 KB |
| Combined kq + kqv_out captures (no-R experimental) | `evals/aloepri-attacks/results/sweep/cell-qwen3-4b-uvo-PAPERLIT-attn-and-output-512-20260526/captures/` | 2.3 GB |
...
| Layer | Plain row | Default obf row | No-R experimental obf row | Plain vocab | Default obf vocab |
|---:|---:|---:|---:|---:|---:|
...
|---|---|---|
| **No-R experimental Alg2 (A1+A2)** vs our deployed Alg2 | −7 to −40 pp depending on surface × layer | This work — same plain captures, only obf construction differs |
...
| **Compound** (no-R experimental × vocab-disjoint) | Predicted ~0 % on both plain and obf at L≥5 across surfaces | **Not measured this session** — no-R experimental × vocab-disjoint not run on either surface yet |

What remains unexplained: paper Table 4's **87.14 % "Noise+KeyMat" baseline**. Ridge under row-split with no-R experimental Alg2 still shows only 43.22 % plain on `kq` at L=0 (much less than 87 %). The 87 % almost certainly requires either a different baseline cell (paper "Noise+KeyMat" lacks the full Alg2 — i.e., it's α_e + KeyMat alone, no head/block perm) OR a stronger attack class than ridge. Step 3 (paper-faithful gradient-opt ISA) remains the unresolved test.
...
| `python/aloepri-llm/obfuscate_qwen3_gguf.py` | Added `alg2_paper_literal` parameter + `--alg2-paper-literal-k-no-r` CLI flag |
| `docs/research/aloepri-attacks.md` | Filled in 2B.1 row + per-layer table (morning session); new no-R experimental results to be added |
| `docs/prototype/aloepri-llm.html` §08 ISA AttnScore | Reframed verdict around §5.4 scope + 2B.1 measurements (morning session); no-R experimental update pending |
...
## Next steps — refocused on obfuscation vs no-R experimental
...
### 1. Compound no-R experimental × vocab-disjoint ridge (highest leverage, cheap)

Run `gpu_sweep.py --split vocab` against the new no-R experimental captures, both surfaces. ~10 min serial each. If it lands at ~0 % on both plain and obf at L≥5 (very likely), the paper Table 4 0 % reproducibility story is closed at the construction + methodology compound level. ~20 min total.
...
### 3. Accuracy / quality impact of no-R experimental Alg2

Paper claims 0–3 % accuracy loss on the bench suite. Our deployed Alg2 is accuracy-validated; no-R experimental has NEVER been accuracy-validated on Qwen3 (only score-surface validated). Run:
- HumanEval pass@1 on the no-R experimental cell (same harness as `evals/aloepri-attacks/m2_7/run_quality_humaneval.py`)
- Spot-check generated text on 10–20 prompts for coherence

If no-R experimental collapses accuracy on Qwen3 (e.g., due to higher-condition-number Û_vo⁻¹ at bf16 multiplication), our deployed Alg2's deviations are *correct deployment hygiene* not bugs. Worth knowing.

### 4. Whether no-R experimental Alg2 can be salvaged by re-introducing minimal stabilisation
...
- `/diagnose` on the construction question — specifically attribute A1 vs A2 defense contribution, then verify accuracy preservation under no-R experimental.
- `/code-review` before the deployed-cell migration if the new no-R experimental becomes the recommended config (load-bearing change in obfuscation construction across multiple deployment paths).
