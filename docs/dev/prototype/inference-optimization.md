...
1. ~~**Keep the unmask on the GPU.** Treat `Aᵀ` as a weight, register
   it per-layer, run `Aᵀ · masked_out` as one more GEMM. Then there's
   no need to read back until after the final pool. Mask is then
   visible to the engine for a brief instant per layer — but it's
   *one fresh mask per batch*, generated by the TEE's CSPRNG, never
   reused, so the engine seeing it post-hoc tells it nothing about
   `H`.~~

   > **❌ STRUCK 2026-05-18 — threat-model-incompatible under
   > the openweight + commodity-GPU model.** The freshness-per-batch
   > argument only protects against **across-forward** correlation; it
   > does nothing for **within-forward** content recovery. If the GPU
   > has `Aᵀ` and `masked = A·H` in the same compute fabric, computing
   > `H = Aᵀ · masked` is the same primitive that does the legitimate
   > unmask — a hostile GPU recovers plaintext `H` for the current
   > forward in microseconds. Layer-1 `H` is the token-embedding lookup
   > of the prompt, so this directly leaks the prompt (and any RAG
   > context inside it) in cleartext. Additionally puts `A` on the
   > PCIe bus by construction, defeating the TEE.Fail-class survival
   > argument in `gelo-llm.html` §03. Only viable under a confidential-
   > GPU threat model (H100 CC mode); GELO explicitly targets
   > commodity Vulkan via VFIO, so on-GPU unmask is unsafe and not a
   > v1 lever. Long-context wins come from **M1.10 fused permuted
   > attention** instead (mask stays in-TEE; what crosses PCIe is the
   > OutAttnMult / permutation-shielded form, not `A`).
...
Option 3 was the right baseline; option 1 is struck. Long-context
mask-round-trip cost (the actual bottleneck at n ≥ 1024 on Qwen3-1.7B —
see §08 of `gelo-llm.html`) is addressed by **M1.10 fused permuted
attention**, not by uploading the mask to the GPU.
...
change. `docs/dev/prototype/gelo.md` lists query confidentiality as a
...
- `crates/gelo-embedder/src/bert/embedder.rs` — add
  ...
  through `embed`.
- `docs/dev/prototype/gelo.md` — document the query-confidentiality
  exception.
...
| 3.4 | ~~On-GPU unmask~~ — **STRUCK 2026-05-18** (threat-model-incompatible — see §2.2.1 callout). Long-context mask cost is addressed by M1.10 fused permuted attention instead. | — | — |
...
- `docs/dev/prototype/gelo.md` §3.4 — the GELO "bandwidth-limited TEE"
  variant that motivates on-GPU unmask.
- `docs/dev/prototype/future-rnd.md` — where block-diagonal mask sampling
