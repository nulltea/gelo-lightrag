...
This resolves the core ambiguity: **true paper K is a real defense lever**, especially on the §5.4-bounded `kqv_out` surface. The old no-R variant is a stronger beyond-paper hardening on raw `kq`, particularly at L=5. But the strongest raw-score case remains L=0: true-paper-K only moves `kq` from 42.3 % to 40.1 % there. Practical reading: paper-aligned AloePri can substantially protect attention outputs, but raw L=0 score capture needs either beyond-paper score hardening, first-layer TEE/path-1, or a runtime policy that does not expose pre-softmax scores.
...
4. **A different threat-model reading.** The AloePri score-surface attack we previously characterised as "AloePri provides ~0 pp defense" was a measurement of the *anti-defense version* of Alg2 we'd deployed. The no-R variant defends meaningfully on this surface too. The remaining 6-7 % obf TTRSR at L≥5 is the operational leak budget, not 47 %.
